...
@bot.event
async def on_ready():
    print('We have logged in as {0.user}, '.format(bot), end = '')
...
async def shorten(message, *args):
  if len(args) == 0:
    await message.send('Please a provide _atleast one link_ to generate short link\n**Syntax**:\n```$shorten link1 ...```')
    return
  for arg in args:
    url = getshorturl(arg)
    await message.send(f'Shortened {arg} -> {url}\n')
  print(f'Generated {len(args)} shortened links\nRequested by: {message.guild.name}, {message.author.name}')
